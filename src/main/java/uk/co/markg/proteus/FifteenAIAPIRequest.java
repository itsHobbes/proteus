...
public class FifteenAIAPIRequest {
  ...
  public CompletableFuture<String> getAudio() throws JsonProcessingException {
    ...
    HttpRequest req =
        HttpRequest.newBuilder().uri(URI.create("https://api.15.ai/app/getAudioFile3"))
            .timeout(Duration.ofMinutes(2)).header("Content-Type", "application/json")
            .POST(BodyPublishers.ofString(request.toJsonString())).setHeader("User-Agent",
                "Proteus Discord bot, Source: https://github.com/itsHobbes/proteus")
            .build();

    return client.sendAsync(req, BodyHandlers.ofString()).thenApply(HttpResponse::body)
